...
\section{Overview of USB Power Delivery}
 \begin{table}[htb]
    \centering
    \caption{USB Specifications and Their Maximum Voltage, Current, and Power \cite{usb_3.2_products}}
...
Starting with USB 2.0, it has a maximum voltage of 5V, a maximum current of 500mA, and a maximum power output of 2.5W. This specification was sufficient for early USB devices like keyboards and mice, which required minimal power. USB 3.0 and 3.1 specifications also maintain a maximum voltage of 5V but increase the maximum current to 900mA, resulting in a maximum power output of 4.5W. This enhancement allowed for faster data transfer rates and supported more power-hungry devices compared to USB 2.0.
...
\end{figure}
The USB Type-C connector features additional pins in contrast to USB Type-A and Type-B connectors\cite{ahmad2020concept}. These pins facilitate USB Type-C functionalities like increased power, Alternate Mode, and reversibility. Figure\ref{fig:typec_plug} displays the pinout.

From left to right \ref{fig:typec_plug} shows:
 \begin{enumerate}
     \item GND: ground aka return path for the signal.
     \item TX/RX: SuperSpeed\texttrademark differential pairs for USB 3.1 data (5 -- 10 Gbps).
     \item VBUS: the main voltage bus (5 V -- 20V).
     \item CC1 and CC2: These CC lines are used for detecting cables, determining orientation, and advertising current. Under USB PD, they can also be utilized for higher power communication and Alternate Mode. Additionally, one of the CC lines might serve as VCONN.
     \item SBU1/SBU2: these low-speed lines are used only for Alternate Mode and accessory mode. "For example, with DisplayPort, AUX+/AUX– transmit over the SBU lines. For audio adapter accessory mode, these lines are used for the microphone input and analog GND"\cite{enos2022primer}.
     \item D+/D–: a high-speed diffrential pair for USB data transfer (up to 480 Mbps).
 \end{enumerate}
 “It is observable that the GND and VBUS lines are still in the same position. The D+ / D– pair is in the same orientation; however, the plug contains only one D+ / D– twisted pair. The USB Type-C specification allows for the coupling of D+ / D- lines (D+ to D+ and D– to D–) on the receptacle side. Regardless of cable orientation, the PHY will always see the D+ / D- pair of the cable"\cite{enos2022primer}. 
 
 “The CC1 and CC2 lines are flipped and can determine the orientation of the cable. The orientation determines which CC line is connected and which one is left open. The TX/RX pairs are also flipped. Resolving this was a bit more complicated. Unlike the D+/D– lines, you cannot simply short the common lines together, because that will create a stub. At USB 2.0 speeds, a stub is acceptable, but at USB 3.1 speeds, a stub degrades signal integrity too much"\cite{enos2022primer}.

 \subsection{Power Delivery}
 “Applications with increasing complexity require USB PD. As mentioned in the introduction, systems with USB PD can support power levels of up to 20 V and 5 A (100 W). This is possible by first increasing the voltage on VBUS while holding the maximum current at 3 A"\cite{enos2022primer}. After reaching the maximum voltage of 20 V, you can increase the current up to 5 A, as seen in Figure\ref{fig:typec_power}.
...
\begin{figure}[hbt]
    \centering
    \includegraphics[width=1\linewidth]{Chapter2/pd_cap_dev.png}
...
\begin{enumerate}
    ...
    \item VCONN Source
\end{enumerate}
UFPs, or Upstream Facing Ports, have several key characteristics. They are designed to sink power and communicate using SOP packets. In addition to these functions, UFPs can optionally source power, making them a Dual-Role Power Device. They also have the capability to communicate via USB and support Alternate Modes\cite{uma1}.

DFPs, or Downstream Facing Ports, also have a range of features. They are primarily designed to source power and communicate using SOP packets. DFPs can also optionally sink power, making them a Dual-Role Power Device as well. They have the option to communicate via USB and support Alternate Modes\cite{uma2}.

The source in this context can take several forms. It can be an externally powered source, such as an AC powered device. Alternatively, it can be a power storage device like a battery or power bank. It can also be derived from another port, such as a bus-powered hub\cite{bsk1}.

A sink, on the other hand, can also be a power storage device like a battery or power bank. It can be used to power internal functions or to power devices attached to other devices, such as a bus-powered hub\cite{Shy1}\cite{Skb1}.

Lastly, a VCONN source has its own set of characteristics. It can be either port partner, either the DFP/UFP or Source/Sink. It powers the cable plug(s) and VPDs, or VCONN Powered Devices. Importantly, it is the only port allowed to talk to the cable plug(s) at any given time. This ensures a clear and uninterrupted communication pathway\cite{hvr1}\cite{hvr2}.
\vspace{3cm}
%\section{LDMOS and GaN HEMTs : A Comparison of Power MOSFETs}
This chapter delves into the fundamentals of USB Power Delivery (USB PD). USB has evolved from a simple data interface to a robust power delivery system, with USB PD enabling up to 100W of power. USB PD allows for dynamic power negotiation between devices, ensuring efficient and safe power delivery by adjusting voltage and current as needed. USB PD specification defines how devices negotiate power levels, ensuring compatibility and safety across different devices and generations.